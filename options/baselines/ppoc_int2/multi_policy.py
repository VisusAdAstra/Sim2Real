...
from baselines.ppoc_int2.distributions1 import make_pdtype
import  pdb


def dense3D2(x, size, name, option, num_options=1, weight_init=None, bias=True):
    w = tf.compat.v1.get_variable(name + "/w", [num_options, x.get_shape()[1], size], initializer=weight_init)
...
class MultiPolicy(object):
    ...
    def _init(self, ob_space, ac_space, kind, num_options=2, dc=0, w_intfc=True):
        assert isinstance(ob_space["image"], gym.spaces.Box)
        ...
        x = ob / 255.0
        state = tf.identity(x)
        state = tf.concat(values=[state[:,0,0], state[:,0,1], state[:,1,0], \
        ...
        if kind == 'small':  # from A3C paper
            ...
            x = U.flattenallbut0(x)
            hidden = tf.nn.relu(tf.compat.v1.layers.dense(x, 494, name='lin', kernel_initializer=U.normc_initializer(1.0)))
            hidden = tf.concat(values=[hidden, state], axis = -1)
        else:
        ...
        termination_sample = tf.greater(self.tpred, tf.random.uniform(shape=tf.shape(input=self.tpred), maxval=1.))
