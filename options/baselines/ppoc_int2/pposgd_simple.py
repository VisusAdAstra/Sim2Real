...
from baselines.ppoc_int2.mpi_adam1 import MpiAdam
...
def learn(env, policy_func, *,
    ...
    pi_I = pi.intfc * pi_w / tf.expand_dims(tf.reduce_sum(input_tensor=pi.intfc * pi_w,axis=1),1)
    pi_I = tf.clip_by_value(pi_I,1e-6,1-1e-6)
    int_loss = - tf.reduce_sum(input_tensor=betas *tf.reduce_sum(input_tensor=pi_I * option_hot,axis=1)    * op_adv)

    intfc = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None,num_options])
    pi_I = intfc * pi.op_pi / tf.expand_dims(tf.reduce_sum(input_tensor=intfc * pi.op_pi,axis=1),1)
    pi_I = tf.clip_by_value(pi_I,1e-6,1-1e-6)
    op_loss = - tf.reduce_sum(input_tensor=betas *tf.reduce_sum(input_tensor=pi_I * option_hot,axis=1)    * op_adv)
    ...
    while True:
        ...
        for opt in range(num_options):
            ...
            optim_epochs = np.clip(np.int8(10 * (indices.size / (timesteps_per_batch/num_options))),10,10) if num_options > 1 else optim_epochs
