...
def sum(x, axis=None, keepdims=False):
    ...
    return tf.reduce_sum(input_tensor=x, axis=axis, keepdims=keepdims)

def mean(x, axis=None, keepdims=False):
    axis = None if axis is None else [axis]
    return tf.reduce_mean(input_tensor=x, axis=axis, keepdims=keepdims)
...
def max(x, axis=None, keepdims=False):
    axis = None if axis is None else [axis]
    return tf.reduce_max(input_tensor=x, axis=axis, keepdims=keepdims)

def min(x, axis=None, keepdims=False):
    axis = None if axis is None else [axis]
    return tf.reduce_min(input_tensor=x, axis=axis, keepdims=keepdims)
...
def categorical_sample_logits(X):
    ...
    return argmax(X - tf.math.log(-tf.math.log(U)), axis=1)
...
def set_value(v, val):
    ...
    if v in VALUE_SETTERS:
        ...
        set_endpoint = tf.compat.v1.placeholder(v.dtype)
...
def file_writer(dir_path):
    ...
    return tf.compat.v1.summary.FileWriter(dir_path, get_session().graph)
...
def dense(x, size, name, weight_init=None, bias=True):
    ...
    ret = tf.matmul(x, w)
    if bias:
        b = tf.compat.v1.get_variable(name + "/b", [size], initializer=tf.compat.v1.zeros_initializer())
        return ret + b
    else:
        return ret

def wndense(x, size, name, init_scale=1.0):
    v = tf.compat.v1.get_variable(name + "/V", [int(x.get_shape()[1]), size],
                        initializer=tf.compat.v1.random_normal_initializer(0, 0.05))
    g = tf.compat.v1.get_variable(name + "/g", [size], initializer=tf.compat.v1.constant_initializer(init_scale))
    b = tf.compat.v1.get_variable(name + "/b", [size], initializer=tf.compat.v1.constant_initializer(0.0))
...
def dropout(x, pkeep, phase=None, mask=None):
    mask = tf.floor(pkeep + tf.random.uniform(tf.shape(input=x))) if mask is None else mask
    if phase is None:
        return mask * x
    else:
...
class Module(object):
    ...
    def __call__(self, *args):
        ...
        with tf.compat.v1.variable_scope(self.name, reuse=not self.first_time):
...
def lengths_to_mask(lengths_b, max_length):
    ...
    """
    lengths_b = tf.convert_to_tensor(value=lengths_b)
