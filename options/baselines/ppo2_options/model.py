...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        for o in range(noptions):
            ALL_OPTIONS.append(tf.compat.v1.placeholder(tf.int32, [None]) )
            ALL_VF_OPTIONS.append(tf.compat.v1.placeholder(tf.int32, [None]) )
            CURR_O_PROB.append(tf.compat.v1.placeholder(tf.float32, [None]))
            LOG_RATIO.append(tf.compat.v1.placeholder(tf.float32, [None]))
            NOT_START_EP.append(tf.compat.v1.placeholder(tf.int32, [None]) )
        

        # CREATE THE PLACEHOLDERS for Policy Over Options 
        # self.O = O = train_model.pdtype_op.sample_placeholder([None])
        self.O = O = tf.compat.v1.placeholder(tf.int32, [None])
        self.ADV_OP = ADV_OP = tf.compat.v1.placeholder(tf.float32, [None])
        self.BETAS = BETAS = tf.compat.v1.placeholder(tf.float32, [None])
        # Keep track of old actor
        self.OLDNEGLOGPOP = OLDNEGLOGPOP = tf.compat.v1.placeholder(tf.float32, [None])


        # CREATE THE PLACEHOLDERS for Termination function
        self.T = T = train_model.pdtype.sample_placeholder([None])
        self.ADV_T = ADV_T = tf.compat.v1.placeholder(tf.float32, [None])
        # Keep track of old actor
        self.OLDNEGLOGP_T = OLDNEGLOGP_T = tf.compat.v1.placeholder(tf.float32, [None])
        ...
        # Final PG loss
        pg_loss_op = tf.reduce_mean(input_tensor=tf.maximum(pg_losses_op, pg_losses2_op))
        entropy_op = tf.reduce_mean(input_tensor=train_model.pd_op.entropy())
        ...
        for i,(pd,pd_t) in enumerate(zip(train_model.pds,train_model.pds_term)):
            ...
            # Final PG loss
            pg_loss = tf.reduce_mean(input_tensor=tf.maximum(pg_losses, pg_losses2)  * CURR_O_PROB[i] )
            entropy = tf.gather(pd.entropy(), ALL_VF_OPTIONS[i])
            entropy = tf.reduce_mean(input_tensor=entropy)
            ...
            # Final term loss
            term_loss = tf.reduce_mean(input_tensor=tf.maximum(term_losses, term_losses2))
            entropy_t = tf.gather(pd_t.entropy(), NOT_START_EP[i])
            entropy_t = tf.reduce_mean(input_tensor=entropy_t)
